...
async function addComment() { if (!comment.value.name.trim() || comment.value.content.trim().length < 2 || comment.value.content.length > 2000) { commentStatus.value = '请填写昵称，评论需为 2-2000 个字'; return; } const payload = { postSlug: post.value.slug, authorName: comment.value.name.trim(), content: comment.value.content.trim() }; try { await submitComment(payload); } catch { const nextComments = [{ ...payload, id: Date.now(), status: 'PENDING', createdAt: new Date().toISOString() }, ...comments.value]; comments.value = nextComments; localStorage.setItem(`signal-comments-${post.value.slug}`, JSON.stringify(nextComments)); } comment.value = { name: '', content: '' }; commentStatus.value = '评论已提交，审核通过后会显示'; }
</script>
