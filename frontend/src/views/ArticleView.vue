...
function progress() {
  ...
  const current = [...article.value.headings].reverse().find((heading) => document.getElementById(heading.id)?.getBoundingClientRect().top <= 180);
  activeHeading.value = current?.id || article.value.headings[0]?.id || '';
...
function setupArticleInteractions() {
  ...
  contentRef.value.querySelectorAll('pre').forEach((pre) => {
    ...
    const button = document.createElement('button'); button.className = 'code-copy'; button.type = 'button'; button.setAttribute('aria-label', '复制代码'); button.title = '复制代码'; button.textContent = '复制代码';
...
<template>
  <div class="blog-shell"><BlogHeader />
    ...
    <main v-else-if="post" class="article-page">
      <div class="article-progress-bar" :style="{ '--article-progress': `${readingProgress}%` }" role="progressbar" aria-label="文章阅读进度" aria-valuemin="0" aria-valuemax="100" :aria-valuenow="Math.round(readingProgress)"><span /></div>
      ...
      <header class="article-header"><div class="article-category">{{ post.category }}</div><h1>{{ post.title }}</h1><p>{{ post.excerpt }}</p><div class="article-byline"><PixelAvatar :name="author.name" :avatar-url="author.avatarUrl" :size="42" /><div><strong>{{ author.name }}</strong><span>{{ formatDate(post.publishedAt) }} · {{ post.readMinutes }} 分钟阅读</span></div><div class="article-stats"><span>{{ post.views }} 阅读</span><span>更新于 {{ post.updatedAt }}</span></div><SharePoster :post="post" /></div></header>
