<script setup>
import {
  Activity, Archive, ArrowLeft, ArrowRight, AudioWaveform, BarChart3, Check, ClipboardList, Eye, FileText,
...
import { createPostAuthor, displayAdminIdentity } from '../authorIdentity';
...
const currentUser = ref({ id: null, loginName: '', name: '', avatarUrl: '', role: 'VIEWER', status: 'ACTIVE' });
const editor = ref({});
...
async function uploadAvatar(event) { const file = event.target.files?.[0]; if (!file) return; try { const body = new FormData(); body.append('file', file); const uploaded = await adminRequest('/admin/media', { method: 'POST', headers: {}, body }); media.value.unshift(uploaded); currentUser.value.avatarUrl = uploaded.url; flash('头像已上传，请保存账户资料'); } catch (error) { flashError(error.message || '头像上传失败'); } finally { event.target.value = ''; } }
...
async function saveAccountProfile() { const name = currentUser.value.name?.trim(); if (!name) return flashError('账户姓名不能为空'); accountSaving.value = true; try { currentUser.value = await adminRequest('/admin/account/profile', { method: 'PUT', body: JSON.stringify({ name, avatarUrl: currentUser.value.avatarUrl || '' }) }); settings.value.authorName = currentUser.value.name; flash('账户姓名和头像已更新'); } catch (error) { flashError(error.message || '账户资料更新失败'); } finally { accountSaving.value = false; } }
  ...
  <div v-else class="admin-shell" :class="{ 'has-error-notice': savedTone === 'error', 'has-info-notice': savedTone === 'info' }">
    ...
    <div class="admin-body"><header class="admin-topbar"><button class="admin-menu-toggle" title="打开导航" aria-label="打开导航" @click="menuOpen = !menuOpen"><X v-if="menuOpen" :size="19" /><Menu v-else :size="19" /></button><div class="admin-breadcrumb">ADMIN <span>/</span> SIGNAL NOTES</div><div class="admin-top-actions"><span class="admin-online">{{ apiStatus }}</span><span class="admin-user"><PixelAvatar :name="displayAdminIdentity(currentUser)" :avatar-url="currentUser.avatarUrl" :size="30" />{{ displayAdminIdentity(currentUser) }} · {{ currentUser.role }}</span></div></header>
      ...
      <main v-else class="admin-content">
        ...
        <template v-else-if="section === 'media'"><header class="admin-page-header"><div><span>LIBRARY / MEDIA</span><h1>媒体库</h1><p>上传、预览、替换或删除图片；被文章或账户头像引用的图片会受到保护。</p></div><label class="button button-primary"><Upload :size="16" />上传文件<input type="file" hidden multiple accept="image/jpeg,image/png,image/webp" @change="uploadMedia" /></label></header><div v-if="saved" class="admin-success">{{ saved }}</div><section class="media-grid"><article v-for="item in media" :key="item.id"><div class="media-thumb"><img :src="mediaPreviewUrl(item)" :alt="item.altText || item.filename" /></div><div class="media-fields"><label>文件名<input v-model="item.filename" /></label><label>替代文本<input v-model="item.altText" /></label><span>{{ item.mimeType }} · {{ item.size }} bytes</span><span class="media-usage" :class="{ 'is-blocked': !item.deletable }">{{ item.avatarReference ? '正在用作账户头像，需先更换或清除头像' : item.referenceCount ? `被 ${item.referenceCount} 篇文章引用，需先替换封面` : '未被引用，可以删除' }}</span><div class="media-card-actions"><button class="icon-button" title="保存媒体信息" aria-label="保存媒体信息" @click="saveMedia(item)"><Save :size="15" /></button><label class="icon-button" title="替换图片文件" aria-label="替换图片文件"><RefreshCw :size="15" /><input type="file" hidden :accept="item.mimeType" @change="replaceMedia(item, $event)" /></label><button class="icon-button danger" :disabled="!item.deletable" :title="item.deletable ? '删除媒体及磁盘文件' : item.avatarReference ? '正在用作账户头像，暂不能删除' : `被 ${item.referenceCount} 篇文章引用，暂不能删除`" aria-label="删除媒体" @click="deleteMedia(item)"><Trash2 :size="15" /></button></div></div></article><p v-if="!media.length" class="admin-empty">还没有上传媒体。</p></section></template>
        ...
        <div class="admin-operation-strip" aria-live="polite"><span :aria-busy="saving">{{ saving ? '文章保存中' : '' }}</span><span :aria-busy="settingsSaving">{{ settingsSaving ? '设置保存中' : '' }}</span><span :aria-busy="accountSaving">{{ accountSaving ? '账户资料保存中' : '' }}</span><span :aria-busy="mailTestStatus.pending">{{ mailTestStatus.pending ? '邮件测试中' : '' }}</span></div>
        <AdminAdvancedCopy v-if="section === 'settings' && !adminLoading && !adminLoadError" :settings="settings" />
        <section v-if="section === 'settings' && !adminLoading && !adminLoadError" class="admin-panel settings-panel account-identity-panel">
          <div class="panel-heading"><div><span>ACCOUNT / IDENTITY</span><h2>作者身份</h2></div><small>新文章会自动绑定当前账户。</small></div>
          <form class="identity-form" :aria-busy="accountSaving" @submit.prevent="saveAccountProfile">
