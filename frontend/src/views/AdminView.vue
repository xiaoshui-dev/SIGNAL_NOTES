...
function flash(message) { saved.value = message; }
function syncEditor() {
...
async function loadAdminData() {
  try {
    ...
    settings.value = { ...settings.value, ...settingData };
    ...
    syncEditor();
  } catch (error) { apiStatus.value = '后端未连接'; flash(error.message || '后台数据加载失败，请检查后端服务'); }
}
...
async function savePost(status = editor.value.status, silent = false) {
  ...
  } catch (error) { apiStatus.value = '后端未连接'; flash(error.message || '保存失败，请检查后端服务'); }
...
watch(editor, () => {
  ...
  clearTimeout(autosaveTimer); flash('有未保存修改'); autosaveTimer = setTimeout(() => savePost('DRAFT', true), 1200);
}, { deep: true });
...
async function deletePost(post) { const permanent = post.status === 'TRASHED'; if (!confirm(permanent ? '永久删除后无法恢复，确定继续吗？' : '确定将文章移入回收站吗？')) return; try { await adminRequest(`/admin/posts/${post.id}${permanent ? '?permanent=true' : ''}`, { method: 'DELETE' }); if (permanent) adminPosts.value = adminPosts.value.filter((item) => item.id !== post.id); else post.status = 'TRASHED'; flash(permanent ? '文章已永久删除' : '文章已移入回收站'); } catch (error) { flash(error.message || '文章删除失败'); } }
async function restorePost(post) { try { Object.assign(post, await adminRequest(`/admin/posts/${post.id}/restore`, { method: 'POST' })); flash('文章已恢复为草稿'); } catch (error) { flash(error.message || '文章恢复失败'); } }
async function bulkStatus(status) { if (!selectedPosts.value.length) return; try { await adminRequest('/admin/posts/batch', { method: 'PATCH', body: JSON.stringify({ ids: selectedPosts.value.map(Number), status }) }); adminPosts.value.forEach((post) => { if (selectedPosts.value.map(String).includes(String(post.id))) post.status = status; }); flash(`已更新 ${selectedPosts.value.length} 篇文章`); selectedPosts.value = []; } catch (error) { flash(error.message || '批量操作失败'); } }
...
async function restoreRevision(id) { if (!confirm('恢复此版本会创建新的草稿版本，继续吗？')) return; try { Object.assign(editor.value, await adminRequest(`/admin/posts/${editId.value}/revisions/${id}/restore`, { method: 'POST' })); flash('已恢复为新草稿'); await loadRevisions(); } catch (error) { flash(error.message || '恢复失败'); } }

async function createCategory() { if (!taxonomyDraft.value.name.trim()) return; try { categoriesAdmin.value.push(await adminRequest('/admin/categories', { method: 'POST', body: JSON.stringify(taxonomyDraft.value) })); taxonomyDraft.value = { name: '', slug: '', description: '' }; flash('分类已创建'); } catch (error) { flash(error.message || '分类创建失败'); } }
async function createTag() { if (!tagDraft.value.name.trim()) return; try { tagsAdmin.value.push(await adminRequest('/admin/tags', { method: 'POST', body: JSON.stringify(tagDraft.value) })); tagDraft.value = { name: '', slug: '', description: '' }; flash('标签已创建'); } catch (error) { flash(error.message || '标签创建失败'); } }
async function saveCategory(item) { try { Object.assign(item, await adminRequest(`/admin/categories/${item.id}`, { method: 'PUT', body: JSON.stringify(item) })); flash('分类已更新'); } catch (error) { flash(error.message || '分类更新失败'); } }
async function saveTag(item) { try { Object.assign(item, await adminRequest(`/admin/tags/${item.id}`, { method: 'PUT', body: JSON.stringify(item) })); flash('标签已更新'); } catch (error) { flash(error.message || '标签更新失败'); } }
async function removeCategory(item) { if (!confirm(`确定删除分类“${item.name}”？`)) return; try { await adminRequest(`/admin/categories/${item.id}`, { method: 'DELETE' }); categoriesAdmin.value = categoriesAdmin.value.filter((value) => value.id !== item.id); flash('分类已删除'); } catch (error) { flash(error.message || '分类删除失败'); } }
async function removeTag(item) { if (!confirm(`确定删除标签“${item.name}”？`)) return; try { await adminRequest(`/admin/tags/${item.id}`, { method: 'DELETE' }); tagsAdmin.value = tagsAdmin.value.filter((value) => value.id !== item.id); flash('标签已删除'); } catch (error) { flash(error.message || '标签删除失败'); } }

async function uploadMedia(event) { for (const file of event.target.files || []) { try { const body = new FormData(); body.append('file', file); media.value.unshift(await adminRequest('/admin/media', { method: 'POST', headers: {}, body })); flash('媒体已上传'); } catch (error) { flash(error.message || '媒体上传失败'); } } event.target.value = ''; }
async function saveMedia(item) { try { Object.assign(item, await adminRequest(`/admin/media/${item.id}`, { method: 'PATCH', body: JSON.stringify({ filename: item.filename, altText: item.altText || '' }) })); flash('媒体信息已保存'); } catch (error) { flash(error.message || '媒体信息保存失败'); } }
async function deleteMedia(item) { if (!confirm(`确定删除“${item.filename}”？`)) return; try { await adminRequest(`/admin/media/${item.id}`, { method: 'DELETE' }); media.value = media.value.filter((value) => value.id !== item.id); flash('媒体已删除'); } catch (error) { flash(error.message || '媒体删除失败'); } }

async function moderate(item, status) { const previous = item.status; item.status = status; try { await adminRequest(`/admin/comments/${item.id}`, { method: 'PATCH', body: JSON.stringify({ status }) }); flash('评论状态已更新'); } catch (error) { item.status = previous; flash(error.message || '评论状态更新失败'); } }
async function replyComment(item) { const content = (commentReplies.value[item.id] || '').trim(); if (content.length < 2) return flash('回复至少需要 2 个字'); try { comments.value.push(await adminRequest(`/admin/comments/${item.id}/reply`, { method: 'POST', body: JSON.stringify({ content }) })); commentReplies.value[item.id] = ''; flash('回复已发布'); } catch (error) { flash(error.message || '回复发布失败'); } }
async function deleteComment(item) { if (!confirm('确定删除这条评论？')) return; try { await adminRequest(`/admin/comments/${item.id}`, { method: 'DELETE' }); comments.value = comments.value.filter((value) => value.id !== item.id); flash('评论已删除'); } catch (error) { flash(error.message || '评论删除失败'); } }

async function updateContact(item, status) { const previous = item.status; item.status = status; try { await adminRequest(`/admin/contact-messages/${item.id}`, { method: 'PATCH', body: JSON.stringify({ status }) }); flash('反馈状态已更新'); } catch (error) { item.status = previous; flash(error.message || '反馈状态更新失败'); } }
async function deleteContact(item) { if (!confirm(`确定删除工单 ${item.ticket}？`)) return; try { await adminRequest(`/admin/contact-messages/${item.id}`, { method: 'DELETE' }); contactMessages.value = contactMessages.value.filter((value) => value.id !== item.id); flash('反馈已删除'); } catch (error) { flash(error.message || '反馈删除失败'); } }
async function updateSubscription(item, status) { const previous = item.status; item.status = status; try { await adminRequest(`/admin/subscriptions/${item.id}`, { method: 'PATCH', body: JSON.stringify({ status }) }); flash('订阅状态已更新'); } catch (error) { item.status = previous; flash(error.message || '订阅状态更新失败'); } }
async function deleteSubscription(item) { if (!confirm(`确定删除订阅者 ${item.email}？`)) return; try { await adminRequest(`/admin/subscriptions/${item.id}`, { method: 'DELETE' }); subscriptions.value = subscriptions.value.filter((value) => value.id !== item.id); flash('订阅者已删除'); } catch (error) { flash(error.message || '订阅者删除失败'); } }

async function createUser() { if (!userDraft.value.name.trim() || !userDraft.value.email.trim()) return flash('姓名和邮箱不能为空'); try { users.value.push(await adminRequest('/admin/users', { method: 'POST', body: JSON.stringify(userDraft.value) })); userDraft.value = { name: '', email: '', loginName: '', password: '', role: 'AUTHOR', status: 'INVITED' }; flash('用户已创建'); } catch (error) { flash(error.message || '用户创建失败'); } }
async function saveUser(user) { try { Object.assign(user, await adminRequest(`/admin/users/${user.id}`, { method: 'PUT', body: JSON.stringify(user) })); user.password = ''; flash('用户信息已保存'); } catch (error) { flash(error.message || '用户保存失败'); } }
async function deleteUser(user) { if (!confirm(`确定删除用户 ${user.email}？`)) return; try { await adminRequest(`/admin/users/${user.id}`, { method: 'DELETE' }); users.value = users.value.filter((value) => value.id !== user.id); flash('用户已删除'); } catch (error) { flash(error.message || '用户删除失败'); } }

async function saveSettings() { try { await adminRequest('/admin/settings', { method: 'PUT', body: JSON.stringify(settings.value) }); apiStatus.value = 'MySQL 已连接'; flash('设置已保存，公开页面刷新后生效'); } catch (error) { apiStatus.value = '后端未连接'; flash(error.message || '设置保存失败'); } }
async function testEmail() { if (!emailTestRecipient.value.trim()) return flash('请输入测试邮箱'); try { const result = await adminRequest('/admin/email/test', { method: 'POST', body: JSON.stringify({ to: emailTestRecipient.value.trim() }) }); flash(result.message || '测试邮件已处理'); } catch (error) { flash(error.message || '邮件测试失败'); } }
async function changePassword() { const value = passwordForm.value; if (value.newPassword.length < 10) return flash('新密码至少需要 10 个字符'); if (value.newPassword !== value.confirmPassword) return flash('两次输入的新密码不一致'); try { await adminRequest('/admin/account/password', { method: 'PUT', body: JSON.stringify({ currentPassword: value.currentPassword, newPassword: value.newPassword }) }); logout(); loginError.value = '密码已更新，请使用新密码登录'; } catch (error) { flash(error.message || '密码更新失败'); } }
async function runBackup() { try { flash('正在导出并校验备份'); backupJobs.value.unshift(await adminRequest('/admin/backups', { method: 'POST' })); flash('备份完成，SHA-256 已校验'); } catch (error) { flash(error.message || '备份失败'); } }
</script>
  ...
  <div v-else class="admin-shell">
    ...
    <div class="admin-body"><header class="admin-topbar"><button class="admin-menu-toggle" title="打开导航" aria-label="打开导航" @click="menuOpen = !menuOpen"><X v-if="menuOpen" :size="19" /><Menu v-else :size="19" /></button><div class="admin-breadcrumb">ADMIN <span>/</span> SIGNAL NOTES</div><div class="admin-top-actions"><span class="admin-online"><i />{{ apiStatus }}</span><span class="admin-user">林默 · 管理员</span></div></header>
      ...
      <main v-else class="admin-content">
        ...
        <template v-else-if="section === 'media'"><header class="admin-page-header"><div><span>LIBRARY / MEDIA</span><h1>媒体库</h1><p>上传、预览并维护文件名和图片替代文本。</p></div><label class="button button-primary"><Upload :size="16" />上传文件<input type="file" hidden multiple accept="image/jpeg,image/png,image/webp" @change="uploadMedia" /></label></header><div v-if="saved" class="admin-success">{{ saved }}</div><section class="media-grid"><article v-for="item in media" :key="item.id"><div class="media-thumb"><img :src="item.url" :alt="item.altText || item.filename" /></div><div class="media-fields"><label>文件名<input v-model="item.filename" /></label><label>替代文本<input v-model="item.altText" /></label><span>{{ item.mimeType }} · {{ item.size }} bytes</span><div class="media-card-actions"><button class="icon-button" title="保存媒体信息" aria-label="保存媒体信息" @click="saveMedia(item)"><Save :size="15" /></button><button class="icon-button danger" title="删除媒体" aria-label="删除媒体" @click="deleteMedia(item)"><Trash2 :size="15" /></button></div></div></article><p v-if="!media.length" class="admin-empty">还没有上传媒体。</p></section></template>

        <template v-else-if="section === 'comments'"><header class="admin-page-header"><div><span>COMMUNITY / MODERATION</span><h1>评论审核</h1><p>{{ comments.filter((comment) => comment.status === 'PENDING').length }} 条评论等待处理。</p></div></header><div v-if="saved" class="admin-success">{{ saved }}</div><section class="admin-panel moderation-list"><article v-for="item in comments" :key="item.id"><div class="moderation-meta"><strong>{{ item.authorName }}</strong><span class="risk-tag">{{ item.reportCount ? `举报 ${item.reportCount}` : item.status }}</span></div><p>{{ item.content }}</p><small>文章：{{ item.postSlug }}<template v-if="item.reportReason"> · 原因：{{ item.reportReason }}</template></small><div class="moderation-actions"><button class="button button-primary" @click="moderate(item, 'APPROVED')"><Check :size="15" />通过</button><button class="button" @click="moderate(item, 'SPAM')">标记垃圾</button><button class="icon-button danger" title="删除评论" aria-label="删除评论" @click="deleteComment(item)"><Trash2 :size="15" /></button></div><form class="moderation-reply" @submit.prevent="replyComment(item)"><input v-model="commentReplies[item.id]" maxlength="2000" placeholder="以站点管理员身份回复" /><button class="button">发布回复</button></form></article><p v-if="!comments.length" class="admin-empty">暂无评论。</p></section></template>

        <template v-else-if="section === 'inbox'"><header class="admin-page-header"><div><span>COMMUNITY / INBOX</span><h1>反馈收件箱</h1><p>查看读者提交的主题、内容、工单和处理状态。</p></div></header><div v-if="saved" class="admin-success">{{ saved }}</div><section class="admin-panel inbox-list"><article v-for="item in contactMessages" :key="item.id"><header><div><strong>{{ item.subject }}</strong><span>{{ item.name }} · {{ item.email }}</span></div><time>{{ item.createdAt?.slice(0, 16).replace('T', ' ') }}</time></header><p>{{ item.message }}</p><footer><code>{{ item.ticket }}</code><select :value="item.status" @change="updateContact(item, $event.target.value)"><option value="RECEIVED">待处理</option><option value="IN_PROGRESS">处理中</option><option value="RESOLVED">已解决</option><option value="ARCHIVED">已归档</option></select><a class="icon-button" :href="`mailto:${item.email}?subject=Re: ${encodeURIComponent(item.subject)}`" title="回复邮件" aria-label="回复邮件"><Mail :size="15" /></a><button class="icon-button danger" title="删除反馈" aria-label="删除反馈" @click="deleteContact(item)"><Trash2 :size="15" /></button></footer></article><p v-if="!contactMessages.length" class="admin-empty">还没有收到反馈。</p></section></template>

        <template v-else-if="section === 'subscribers'"><header class="admin-page-header"><div><span>COMMUNITY / SUBSCRIBERS</span><h1>订阅者</h1><p>订阅状态、确认时间和退订记录都来自数据库。</p></div></header><div v-if="saved" class="admin-success">{{ saved }}</div><section class="admin-panel"><div class="subscriber-toolbar"><label>发送测试邮件到<input v-model="emailTestRecipient" type="email" placeholder="your@email.com" /></label><button class="button" @click="testEmail"><Mail :size="15" />测试邮件</button></div><div class="subscriber-list"><div v-for="item in subscriptions" :key="item.id"><div><strong>{{ item.email }}</strong><small>订阅于 {{ item.createdAt?.slice(0, 10) }}<template v-if="item.confirmedAt"> · 确认于 {{ item.confirmedAt.slice(0, 10) }}</template></small></div><select :value="item.status" @change="updateSubscription(item, $event.target.value)"><option value="PENDING">待确认</option><option value="CONFIRMED">已确认</option><option value="UNSUBSCRIBED">已退订</option></select><button class="icon-button danger" title="删除订阅者" aria-label="删除订阅者" @click="deleteSubscription(item)"><Trash2 :size="15" /></button></div><p v-if="!subscriptions.length" class="admin-empty">还没有订阅者。</p></div></section></template>

        <template v-else-if="section === 'users'"><header class="admin-page-header"><div><span>ACCESS / USERS</span><h1>用户与角色</h1><p>管理登录名、密码、角色和状态；停用后该账号立即无法登录。</p></div></header><div v-if="saved" class="admin-success">{{ saved }}</div><section class="admin-panel"><form class="user-create-form" @submit.prevent="createUser"><input v-model="userDraft.name" placeholder="姓名" /><input v-model="userDraft.email" type="email" placeholder="邮箱" /><input v-model="userDraft.loginName" autocomplete="username" placeholder="登录名" /><input v-model="userDraft.password" type="password" autocomplete="new-password" placeholder="初始密码（至少 10 位）" /><select v-model="userDraft.role"><option value="ADMIN">管理员</option><option value="EDITOR">编辑</option><option value="AUTHOR">作者</option><option value="VIEWER">访客</option></select><select v-model="userDraft.status"><option value="ACTIVE">启用</option><option value="INVITED">待邀请</option><option value="DISABLED">停用</option></select><button class="button button-primary"><Plus :size="15" />新增用户</button></form><div class="user-management-list"><div v-for="user in users" :key="user.id"><input v-model="user.name" aria-label="用户姓名" /><input v-model="user.email" type="email" aria-label="用户邮箱" /><input v-model="user.loginName" autocomplete="username" aria-label="登录名" placeholder="未设置" /><input v-model="user.password" type="password" autocomplete="new-password" aria-label="设置新密码" placeholder="留空则不修改" /><select v-model="user.role" aria-label="用户角色"><option value="ADMIN">管理员</option><option value="EDITOR">编辑</option><option value="AUTHOR">作者</option><option value="VIEWER">访客</option></select><select v-model="user.status" aria-label="用户状态"><option value="ACTIVE">启用</option><option value="INVITED">待邀请</option><option value="DISABLED">停用</option></select><span class="quick-action-buttons"><button class="icon-button" title="保存用户" :aria-label="`保存 ${user.email}`" @click="saveUser(user)"><Save :size="15" /></button><button class="icon-button danger" title="删除用户" :aria-label="`删除 ${user.email}`" @click="deleteUser(user)"><Trash2 :size="15" /></button></span></div></div></section></template>

        <template v-else-if="section === 'settings'"><header class="admin-page-header"><div><span>SYSTEM / SETTINGS</span><h1>站点设置</h1><p>公开页面文案、分享模板、邮件投递和账户安全。</p></div><button class="button button-primary" @click="saveSettings"><Save :size="15" />保存全部设置</button></header><div v-if="saved" class="admin-success"><Check :size="15" />{{ saved }}</div><div class="settings-grid"><section class="admin-panel settings-panel public-copy-panel"><div class="panel-heading"><div><span>PUBLIC CONTENT</span><h2>公开页面文案</h2></div></div><div class="public-copy-grid"><label>站点名称<input v-model="settings.siteName" /></label><label>站点短名<input v-model="settings.siteShortName" /></label><label>介绍页眉标<input v-model="settings.heroEyebrow" /></label><label>介绍页标题<input v-model="settings.heroTitle" /></label><label class="wide">介绍页摘要<textarea v-model="settings.heroSummary" rows="3" /></label><label>关于页标题<input v-model="settings.aboutTitle" /></label><label>关于页引导语<textarea v-model="settings.aboutLead" rows="2" /></label><label class="wide">关于页正文<textarea v-model="settings.aboutBody" rows="3" /></label><label>联系页标题<input v-model="settings.contactTitle" /></label><label>公开邮箱<input v-model="settings.publicEmail" type="email" /></label><label class="wide">联系页说明<textarea v-model="settings.contactIntro" rows="2" /></label><label>回复承诺<input v-model="settings.replyPromise" /></label><label class="wide">隐私说明<textarea v-model="settings.privacyContent" rows="3" /></label><label>页脚描述<input v-model="settings.footerDescription" /></label><label>版权文案<input v-model="settings.copyrightText" /></label><label>许可文案<input v-model="settings.licenseText" /></label><label>订阅标题<input v-model="settings.subscribeTitle" /></label><label class="wide">订阅说明<textarea v-model="settings.subscribeDescription" rows="2" /></label></div></section><section class="admin-panel settings-panel"><div class="panel-heading"><div><span>SHARE / POSTER</span><h2>分享海报</h2></div></div><label>默认模板<select v-model="settings.shareTemplate"><option value="landscape">横版卡片</option><option value="portrait">竖版海报</option></select></label><div class="share-setting-preview"><span>{{ settings.siteShortName }}</span><strong>{{ settings.siteName }}</strong><small>{{ settings.heroSummary }}</small><i>QR</i></div></section><section class="admin-panel settings-panel mail-panel"><div class="panel-heading"><div><span>SYSTEM / EMAIL</span><h2>邮件服务</h2></div></div><p class="settings-help">未启用 SMTP 时，订阅和反馈仍会入库，但不会声称邮件已发送。</p><div class="mail-settings-grid"><label class="mail-enabled"><input v-model="settings['mail.enabled']" type="checkbox" true-value="true" false-value="false" />启用 SMTP 投递</label><label>SMTP 主机<input v-model="settings['mail.host']" placeholder="smtp.example.com" /></label><label>端口<input v-model="settings['mail.port']" type="number" /></label><label>用户名<input v-model="settings['mail.username']" /></label><label>密码<input v-model="settings['mail.password']" type="password" autocomplete="new-password" :placeholder="settings['mail.passwordConfigured'] === 'true' ? '已保存，留空则不修改' : '首次填写密码'" /></label><label>发件人<input v-model="settings['mail.from']" type="email" placeholder="noreply@example.com" /></label><label>反馈通知邮箱<input v-model="settings['mail.notificationTo']" type="email" placeholder="owner@example.com" /></label><label class="mail-enabled"><input v-model="settings['mail.starttls']" type="checkbox" true-value="true" false-value="false" />启用 STARTTLS</label><label class="mail-enabled"><input v-model="settings['mail.auth']" type="checkbox" true-value="true" false-value="false" />启用 SMTP 登录</label></div><div class="mail-settings-actions"><input v-model="emailTestRecipient" type="email" placeholder="测试收件地址" /><button class="button" @click="testEmail"><Mail :size="15" />发送测试邮件</button></div></section><section class="admin-panel settings-panel"><div class="panel-heading"><div><span>ACCOUNT / SECURITY</span><h2>修改密码</h2></div></div><form class="account-form" @submit.prevent="changePassword"><label>当前密码<input v-model="passwordForm.currentPassword" type="password" autocomplete="current-password" /></label><label>新密码<input v-model="passwordForm.newPassword" type="password" minlength="10" autocomplete="new-password" /></label><label>确认新密码<input v-model="passwordForm.confirmPassword" type="password" minlength="10" autocomplete="new-password" /></label><button class="button">更新并重新登录</button></form></section></div></template>
        ...
        <AdminAdvancedCopy v-if="section === 'settings'" :settings="settings" />
      </main>
    </div>
  </div>
</template>
