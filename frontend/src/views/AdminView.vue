  ...
  <div v-else class="admin-shell" :class="{ 'has-error-notice': savedTone === 'error', 'has-info-notice': savedTone === 'info' }">
    ...
    <div class="admin-body"><header class="admin-topbar"><button class="admin-menu-toggle" title="打开导航" aria-label="打开导航" @click="menuOpen = !menuOpen"><X v-if="menuOpen" :size="19" /><Menu v-else :size="19" /></button><div class="admin-breadcrumb">ADMIN <span>/</span> SIGNAL NOTES</div><div class="admin-top-actions"><span class="admin-online">{{ apiStatus }}</span><span class="admin-user">{{ currentUser.name || currentUser.loginName || '当前账户' }} · {{ currentUser.role }}</span></div></header>
      ...
      <main v-else class="admin-content">
        ...
        <div class="admin-operation-strip" aria-live="polite"><span :aria-busy="saving">{{ saving ? '文章保存中' : '' }}</span><span :aria-busy="settingsSaving">{{ settingsSaving ? '设置保存中' : '' }}</span><span :aria-busy="mailTestStatus.pending">{{ mailTestStatus.pending ? '邮件测试中' : '' }}</span></div>
