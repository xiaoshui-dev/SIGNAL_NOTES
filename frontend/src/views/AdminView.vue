...
async function saveSettings() { if (adminLoading.value || adminLoadError.value) return;
...
async function testEmail() { if (!/^\S+@\S+\.\S+$/.test(emailTestRecipient.value.trim())) return flashError('请输入有效的测试邮箱'); try { const result = await adminRequest('/admin/email/test', { method: 'POST', body: JSON.stringify({ to: emailTestRecipient.value.trim() }) }); const tone = result.sent ? 'success' : result.configured ? 'error' : 'info'; flash(result.message || '测试邮件已处理', tone); } catch (error) { flashError(error.message || '邮件测试失败'); } }
