...
const topics = computed(() => {
  ...
  const configuredByName = new Map((Array.isArray(configured) ? configured : []).map((item) => [item.name, item]));
  if (!categories.value.length) return Array.isArray(configured) ? configured : [];
  return categories.value.map((category, index) => {
    const saved = configuredByName.get(category.name) || {};
    return { number: String(index + 1).padStart(2, '0'), name: category.name, summary: saved.summary || category.description };
  });
});
...
<template>
  <main class="landing-page">
    <section class="hero" aria-labelledby="hero-title">
      ...
      <div class="hero-status reveal reveal-4"><div><span>最近更新</span><strong>{{ posts[0]?.publishedAt || '—' }}</strong></div><div><span>正在记录</span><strong>{{ site.landingStatusTopics }}</strong></div><a href="#featured">向下探索 <ArrowDown :size="15" /></a></div>
    </section>
    <section id="topics" class="landing-band topic-band"><div class="section-kicker">01 / TOPICS</div><div class="band-heading"><h2>{{ site.landingTopicsTitle }}</h2><p>{{ site.landingTopicsIntro }}</p></div><div class="topic-index"><RouterLink v-for="topic in topics" :key="topic.name" class="topic-row" :to="{ path: '/blog', query: { topic: topic.name } }"><span>{{ topic.number }}</span><strong>{{ topic.name }}</strong><small>{{ topic.summary }}</small><ArrowRight :size="20" /></RouterLink></div></section>
    <section id="featured" class="landing-band featured-band"><div class="section-kicker">02 / SELECTED</div><div class="band-heading compact"><h2>{{ site.featuredTitle }}</h2><RouterLink to="/blog">全部文章 <ArrowRight :size="17" /></RouterLink></div><div v-if="featured.length" class="featured-grid"><RouterLink v-for="(post, index) in featured" :key="post.id" class="feature-story" :class="index === 0 ? 'feature-main' : index === 1 ? 'feature-text' : 'feature-accent'" :to="`/blog/posts/${post.slug}`"><img v-if="index === 0" :src="post.cover" :alt="post.coverAlt" /><div><span>{{ post.category }} · {{ post.readMinutes }} 分钟</span><h3>{{ post.title }}</h3><p v-if="index === 1">{{ post.excerpt }}</p><ArrowRight v-if="index === 2" :size="24" /></div></RouterLink></div><div v-else class="landing-empty"><p>{{ contentStatus }}</p><RouterLink class="button" to="/blog">查看博客</RouterLink></div></section>
