...
<template>
  <div class="blog-shell"><BlogHeader /><main class="blog-main" :class="{ 'compact-main': section }">
    ...
    <template v-else-if="section === 'tags'"><template v-if="routeSlug"><header class="page-heading"><span>TAG / 008</span><h1>#{{ routeSlug }}</h1><p>所有带有这个标签的文章。</p></header><section class="post-list compact-post-list"><article v-for="post in items.filter((item) => item.tags?.includes(routeSlug))" :key="post.id" class="post-card"><div class="post-card-copy"><div class="post-meta"><span>{{ post.category }}</span><span>{{ formatDate(post.publishedAt) }}</span></div><h2><RouterLink :to="`/blog/posts/${post.slug}`">{{ post.title }}</RouterLink></h2><p>{{ post.excerpt }}</p></div></article></section><div v-if="!items.some((item) => item.tags?.includes(routeSlug))" class="empty-state"><span>NO NOTES</span><h2>这个标签下还没有公开文章</h2><p>可以返回标签索引继续浏览。</p><RouterLink class="button" to="/blog/tags">全部标签</RouterLink></div></template><template v-else><header class="page-heading"><span>TAG INDEX / 008</span><h1>{{ site.tagsTitle || '全部标签' }}</h1><p>{{ site.tagsIntro || '从关键词进入相关主题与文章。' }}</p></header><section class="tag-cloud tag-index-page"><div><RouterLink v-for="tag in tags" :key="tag.slug" :to="`/blog/tags/${encodeURIComponent(tag.name)}`">#{{ tag.name }} <small>{{ tag.count }} 篇</small></RouterLink></div><p v-if="!tags.length" class="admin-empty">还没有公开标签。</p></section></template></template>
