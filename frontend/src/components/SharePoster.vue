...
const open = ref(false); const variant = ref('landscape'); const qr = ref(''); const posterUrl = ref(''); const status = ref('');
...
async function generate() {
  ...
  const pad=portrait?76:66; ctx.fillStyle='#b9ff66'; ctx.fillRect(pad,pad,44,4); ctx.fillStyle='rgba(255,255,255,.7)';ctx.font='500 20px monospace';ctx.fillText('SIGNAL NOTES / TECH NOTES',pad,pad+42);
...
async function copy(){await navigator.clipboard?.writeText(`${props.post.title}\n${props.post.excerpt}\n${url.value}`);status.value='链接已复制';setTimeout(()=>status.value='',1800);}
function download(){const a=document.createElement('a');a.href=posterUrl.value;a.download=`脉冲笔记-${props.post.title.slice(0,28)}.png`;a.click();status.value='分享图片已保存';}
</script>

<template>
  <button class="article-share-trigger" type="button" @click="open=true"><Share2 :size="17" />分享文章</button>
  <div v-if="open" class="modal-backdrop" @mousedown.self="open=false"><div class="modal" role="dialog" aria-modal="true"><header><div><span>SHARE / SIGNAL</span><h2>生成分享图片</h2></div><button class="icon-button" @click="open=false"><X :size="18" /></button></header><div class="share-modal-body"><div class="poster-preview"><div class="poster-preview-brand">SIGNAL NOTES</div><img v-if="posterUrl" :src="posterUrl" alt="文章分享海报预览" /><div v-else class="poster-loading"><ImageIcon :size="20" />正在生成图片</div></div><div class="poster-controls"><div class="poster-tabs"><button :class="{active:variant==='landscape'}" @click="variant='landscape'">横版 1200×630</button><button :class="{active:variant==='portrait'}" @click="variant='portrait'">竖版 1080×1440</button></div><div class="share-actions"><button @click="copy"><Clipboard :size="16" />复制链接</button><button :disabled="!posterUrl" @click="download"><Download :size="16" />保存图片</button><button :disabled="!posterUrl" @click="navigator.share?.({title:post.title,text:post.excerpt,url}) || copy()"><Share2 :size="16" />系统分享</button></div><div v-if="qr" class="qr-row"><img :src="qr" alt="文章二维码" /><span>扫码阅读全文<br /><small>二维码指向文章 canonical URL</small></span></div><div v-if="status" class="inline-success"><Check :size="15" />{{ status }}</div></div></div></div></div>
</template>
