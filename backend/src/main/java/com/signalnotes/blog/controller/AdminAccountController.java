...
public class AdminAccountController {
    ...
    @PutMapping("/profile") public Map<String,Object> updateProfile(@Valid @RequestBody ProfileUpdate input,Authentication auth){var user=repository.findByLoginName(auth.getName()).orElseThrow(()->new EntityNotFoundException("用户不存在"));String avatar=input.avatarUrl()==null?null:input.avatarUrl().trim();if(avatar!=null&&!avatar.isBlank()&&!avatar.startsWith("/uploads/"))throw new IllegalArgumentException("头像必须来自本地媒体库");user.setName(input.name().trim());user.setAvatarUrl(avatar==null||avatar.isBlank()?null:avatar);repository.save(user);audit.log(auth.getName(),"UPDATE_ACCOUNT_PROFILE",user.getId().toString());Map<String,Object> value=new java.util.LinkedHashMap<>();value.put("id",user.getId());value.put("loginName",user.getLoginName());value.put("name",user.getName());value.put("avatarUrl",user.getAvatarUrl());value.put("role",user.getRole());value.put("status",user.getStatus());return value;}
