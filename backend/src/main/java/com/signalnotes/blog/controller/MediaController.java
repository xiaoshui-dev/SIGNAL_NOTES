...
public class MediaController {
    ...
    public void delete(@PathVariable Long id,Authentication auth) throws IOException {MediaAsset asset=media.findById(id).orElseThrow(()->new jakarta.persistence.EntityNotFoundException("媒体不存在"));long referenceCount=posts.countByCover(asset.getUrl());if(referenceCount>0)throw new IllegalArgumentException("媒体仍被 "+referenceCount+" 篇文章引用，请先替换这些文章的封面");Path file=uploadDir.resolve(asset.getUrl().replace("/uploads/","")).normalize();if(file.startsWith(uploadDir))Files.deleteIfExists(file);media.delete(asset);audit.log(auth.getName(),"DELETE_MEDIA",id.toString());}
    ...
    private MediaView view(MediaAsset asset){long referenceCount=posts.countByCover(asset.getUrl());return new MediaView(asset.getId(),asset.getFilename(),asset.getUrl(),asset.getMimeType(),asset.getSize(),asset.getAltText(),asset.getCreatedAt(),referenceCount,referenceCount==0);}
    public record MediaView(Long id,String filename,String url,String mimeType,long size,String altText,Instant createdAt,long referenceCount,boolean deletable){}
